...
class Employee:
    ...
    def apply_raise(self, managed_employee: 'Employee', raise_percent: int):
      if managed_employee._manager is not self:
        return f"Error: {managed_employee._name} is not managed by {self._name}"

      old_salary = managed_employee.salary
      managed_employee.salary = round(old_salary * (1 + raise_percent / 100))
      
      print(f"Raise applied successfully! {managed_employee._name}'s salary: "
            f"{managed_employee.salary}")
      return True
